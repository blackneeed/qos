...
qemu-system-x86_64 -cdrom qos.iso -boot d -d guest_errors -debugcon stdio -serial null -vga std -machine q35 -device isa-debug-exit,iobase=0x501,iosize=1 ${FLAGS}
exit $(($? >> 1)) # holy bash
